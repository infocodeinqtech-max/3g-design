...
import { logoutDemo } from "../demoAuth";
import {
...
export default function AdminSidebar() {
  ...
  const standaloneLinks = useMemo(
    () => [...STANDALONE_ASSIGNABLE_MENUS, ...SUPERADMIN_MENUS],
    [],
  );

  const homeSectionLinks = HOME_PAGE_SECTION_GROUP.items;
  ...
  const NavContent = () => (
    <>
      <div className="px-4 pt-6 pb-5 border-b border-[#E8DFD2] flex flex-col items-center">
        <Link
          to="/"
          className="w-full flex items-center justify-center min-h-[5rem]"
        >
          <img
            src={logo}
        ...
        <p className="text-[10px] text-[#B0A69A] mt-1">Design demo</p>
      </div>
